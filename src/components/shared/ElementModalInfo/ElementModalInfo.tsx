...
export default function ElementModalInfo({ element }: ElementModalProps) {
    return (
        <div className="overflow-y-auto max-h-[90rem] min-w-96 border border-neutral-600 bg-neutral-900 rounded-md z-[9999]">
            <RowInfo data={{ value: element.name }} label="Element" />
            <RowInfo data={{ value: element.symbol }} label="Symbol" />
            ...
            <RowInfo data={{ value: element.position.group }} label="Group" />
            <RowInfo data={{ value: element.position.period }} label="Period" />
        </div>
    );
}